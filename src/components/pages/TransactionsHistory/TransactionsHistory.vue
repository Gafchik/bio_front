...
import {useTransactionsHistoryStore} from "@/store/pages/TransactionsHistory/transactions-history-store.js";
import {storeToRefs} from "pinia";
import {computed, ref} from "vue";
import moment from "moment/moment.js";
const TRANC_PREFIX = 'pages.transactions_history'
const {t} = useI18n()
const transactionsHistoryStore = useTransactionsHistoryStore()
const {transactions} = storeToRefs(transactionsHistoryStore)
const {getTransactionAsync} = transactionsHistoryStore
getTransactionAsync()
...
const columns = computed(() => {
  return [
    ...
    },
    {
      name: 'count_trees',
      required: true,
      label: t(`${TRANC_PREFIX}.table_headers.count_trees`),
      align: 'center',
    ...
    },
    {
      name: 'message',
      required: true,
      label: t(`${TRANC_PREFIX}.table_headers.comment`),
      align: 'center',
    ...
    <template v-slot:personal-content>
      ...
      >
        ...
        <template v-slot:body-cell-type="props">
          <q-td class="text-center">
            {{t(`app.transaction_type.${props.row.type}`)}}
          </q-td>
        </template>
        <template v-slot:body-cell-status="props">
          <q-td class="text-center">
            {{t(`app.transaction_status.${props.row.status}`)}}
          </q-td>
        </template>
        <template v-slot:body-cell-message="props">
          <q-td class="text-center">
            <span v-if="!!props.row.message">{{props.row.message}}</span>
            <span v-else></span>
          </q-td>
        </template>
            ...
            <q-list dense style="background-color: #f5f3e4;">
              <q-item v-for="col in props.cols"
                ...
                <template v-if="col.name === 'date'">
                  ...
                  <q-item-section side>
                    <q-item-label caption class="text-black">{{getDate( col.value)}}</q-item-label>
                  </q-item-section>
                </template>
                ...
                <template v-if="col.name === 'type'">
                  ...
                  <q-item-section side>
                    <q-item-label caption class="text-black">
                      {{t(`app.transaction_type.${col.value}`)}}
                    </q-item-label>
                  </q-item-section>
                </template>
                <template v-if="col.name === 'status'">
                  ...
                  <q-item-section side>
                    <q-item-label caption class="text-black">
                      {{t(`app.transaction_status.${col.value}`)}}
                    </q-item-label>
                  </q-item-section>
                </template>
                <template v-if="col.name === 'message'">
                  <q-item-section>
                    <q-item-label class="text-bold">{{ col.label }}</q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <q-item-label caption class="text-black">
                      <span v-if="!!props.row.message">{{col.value}}</span>
                      <span v-else></span>
                    </q-item-label>
                  </q-item-section>
                </template>
                <template v-if="col.name === 'count_trees'">
                  <q-item-section>
                    <q-item-label class="text-bold">{{ col.label }}</q-item-label>
                  </q-item-section>
                  <q-item-section side>
